...
3. 有泛型：List<String> list = new ArrayList<>(); list.add("abc"); String s = list.get(0);

4. **代码复用**：通过类型参数抽象共性逻辑，一套代码适配多种类型。例如：一个泛型工具类 `Utils<T>` 可处理 String、Integer 等不同类型的对象，无需为每种类型写重复代码。

5. **支持通用算法**：结合集合框架，实现通用的排序、过滤等算法，适配不同类型集合。
...
1. 无界泛型（类/接口定义时）：擦除为 Object（如泛型类 List<T> 擦除后为原始类型 List，内部元素按 Object 处理）。

2. 上界泛型（类/接口定义或通配符）：擦除为上界类型（如 List<T extends Number> 或 List<? extends Number>，擦除后均为 List<Number>）。

3. 下界通配符泛型：擦除为 Object（Java 仅支持通配符用 super 指定下界，如 List<? super Integer>，擦除后为 List<Object>；泛型类/接口定义时不能用 super 限定类型参数，因此无此场景）。
...
静态方法加载时，泛型类的类型参数（如上述代码中GenericClass的T）尚未确定（需实例化时才指定），因此静态方法**不能使用类的泛型参数**。若静态方法需要泛型能力，需**单独声明自己的泛型参数**（即静态泛型方法），此时声明的泛型参数与类的泛型参数无关，即使同名（如均用T）也属于不同参数。

```Plain Text


public class GenericClass<T> {
    // 编译报错：静态方法不能使用“类的泛型参数T”
    // public static T getValue() { ... } 
    // 正确：静态方法单独声明“自己的泛型参数T”（与类的T重名但独立）
