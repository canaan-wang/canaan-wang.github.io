...
HashSet 是 Java 集合框架中 `java.util` 包下的实现类，实现 `Set` 接口，间接实现 `Collection` 和 `Iterable` 接口，是基于哈希表实现的**无序、不可重复**集合。

### 1.2 核心特性速览

**不可重复性**：无重复元素，依赖 `hashCode()` 和 `equals()` 实现<br/>
**无序性**：存储与迭代顺序与添加顺序无关，且不保证永久不变<br/>
**允许null**：仅支持存储1个null元素（哈希值固定为0）<br/>
**非线程安全**：并发修改可能导致数据不一致或异常<br/>
**无索引**：无通过索引访问元素的方法（如 `get(int index)`）<br/>
...
2. **判断索引位置**：索引为空时，直接将元素作为键、PRESENT作为值存入，添加成功
3. **哈希冲突处理**：索引非空时，调用 `equals()` 与该位置元素对比
   - equals返回true：元素已存在，添加失败（去重核心）
   - equals返回false：通过链表（JDK1.7及以前）或红黑树（JDK1.8+，链表长度≥8时）解决冲突，挂载新元素
...
#### 5.2.1 自定义对象必须重写 hashCode() 和 equals()
...
#### 5.2.2 线程安全问题
- 多线程环境下可使用 `Collections.synchronizedSet(new HashSet<>())` 包装为同步集合
- JDK 9+ 可通过 `Set.copyOf()` 结合并发 Map 创建并发安全的集合
- 或使用第三方库提供的 `ConcurrentHashSet` 实现

#### 5.2.3 扩容机制影响性能
- 当元素数量超过「当前容量 × 负载因子」时，HashMap 会扩容为原容量的 2 倍
- 扩容会触发所有元素重新计算哈希值和索引，建议初始化时预估容量以减少扩容次数
